...
nodes_raw = CSV.read(data_path*"/dataset/network_data/$data_dir/nodes_multiphase4.csv", DataFrame)
sum(nonunique(nodes_raw, :index)) != 0 ? warn("Ambiguous Node Indices") : nothing

lines_raw_singlephase = CSV.read(data_path*"/dataset/network_data/$data_dir/lines_multiphase.csv", DataFrame)
sum(nonunique(lines_raw_singlephase, :index)) != 0  ? warn("Ambiguous Line Indices") : nothing

lines_raw_multiphase = CSV.read(data_path*"/dataset/network_data/$data_dir/lines_multiphase2.csv", DataFrame)
